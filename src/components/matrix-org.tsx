...
function ET({ value, onChange, style }: { value: string; onChange: (v: string) => void; style?: React.CSSProperties }) {
  ...
  if (editing) return (
    <input ref={ref} value={draft} onChange={e=>setDraft(e.target.value)} onBlur={commit}
      ...
      className="bg-white/[0.08] border border-white/30 rounded text-slate-100 px-1.5 py-0.5 text-[inherit] font-[inherit] w-full min-w-[80px] outline-none"
    />
  );
...
/* ── Editable chip ── */
function Chip({ name, accent, onChange, onDelete, onJD }: { name: string; accent: string; onChange: (v: string) => void; onDelete: () => void; onJD?: () => void }) {
  const [editing, setEditing] = useState(false);
...
/* ── JD Modal ── */
function JDModal({ layerId, layer, onClose }: { layerId: string; layer: any; onClose: () => void }) {
    ...
    >
      <div style={{ background:"#0b1320", border:"1px solid "+accent+"44", borderRadius:"16px", maxWidth:"760px", width:"100%", maxHeight:"85vh", overflow:"hidden", display:"flex", flexDirection:"column" }}>
        ...
        <div style={{ padding:"20px 24px 16px", borderBottom:"1px solid "+accent+"22", display:"flex", alignItems:"flex-start", justifyContent:"space-between", gap:"16px", flexShrink:0 }}>
          ...
          <Button variant="ghost" size="sm" onClick={onClose} className="text-slate-500 text-xl leading-none shrink-0">×</Button>
        </div>
        {/* body scrollable */}
        <div style={{ overflowY:"auto", padding:"20px 24px", display:"flex", flexDirection:"column", gap:"20px" }}>
          ...
          <div>
            ...
            {jd.responsibilities.map((r: any,i: number)=>(
...
/* ── Add-column modal ── */
function AddColModal({ onAdd, onClose }: { onAdd: (data: { name: string; type: string; productLead: string }) => void; onClose: () => void }) {
    ...
    <div style={{ position:"fixed", inset:0, background:"rgba(0,0,0,0.7)", display:"flex", alignItems:"center", justifyContent:"center", zIndex:1000 }}>
      <div style={{ background:"#0f1a24", border:"1px solid #1e293b", borderRadius:"14px", padding:"24px", minWidth:"280px" }}>
        ...
        <div className="flex gap-2">
          ...
          <Button className="flex-1" onClick={()=>onAdd({name:name.trim()||"New Product", type, productLead:lead.trim()||"TBD"})}>Add</Button>
        </div>
      </div>
    </div>
  );
...
/* ── Add-row modal ── */
function AddRowModal({ onAdd, onClose }: { onAdd: (data: any) => void; onClose: () => void }) {
  ...
  return (
    <div style={{ position:"fixed", inset:0, background:"rgba(0,0,0,0.7)", display:"flex", alignItems:"center", justifyContent:"center", zIndex:1000 }}>
      <div style={{ background:"#0f1a24", border:"1px solid #1e293b", borderRadius:"14px", padding:"24px", minWidth:"300px" }}>
        ...
        {([["Layer Name",label,setLabel,ref],["Description",sublabel,setSublabel,null],["Lead Name",leadName,setLeadName,null],["Lead Role",leadRole,setLeadRole,null]] as [string, string, any, any][]).map(([lbl,val,set,r])=>(
          <div key={lbl}>
            <label style={{ fontSize:"11px", color:"#94a3b8", display:"block", marginBottom:"4px" }}>{lbl as string}</label>
            <input ref={r||undefined} value={val as string} onChange={(e: React.ChangeEvent<HTMLInputElement>)=>set(e.target.value)}
              style={{ width:"100%", background:"#0b1320", border:"1px solid #1e293b", borderRadius:"6px", color:"#f1f5f9", padding:"6px 10px", fontSize:"12px", marginBottom:"12px", outline:"none", boxSizing:"border-box" }} />
          </div>
        ))}
        ...
        <div className="flex gap-2">
          <Button variant="secondary" className="flex-1" onClick={onClose}>Cancel</Button>
          <Button className="flex-1" style={{background:accent}} onClick={()=>onAdd({ label:label.trim()||"New Layer", sublabel:sublabel.trim(), lead:{ name:leadName.trim()||"TBD", role:leadRole.trim()||"Layer Lead" }, accent, color })}>Add</Button>
        </div>
      </div>
    </div>
  );
...
/* ── Assign Rows Modal ── */
function AssignRowsModal({ exec, layers, onSave, onClose }: { exec: any; layers: any[]; onSave: (v: string[]) => void; onClose: () => void }) {
  const [selected, setSelected] = useState<Set<string>>(new Set(exec.layers as string[]));
  ...
  return (
    <div style={{ position:"fixed", inset:0, background:"rgba(0,0,0,0.8)", display:"flex", alignItems:"center", justifyContent:"center", zIndex:3000, padding:"20px" }}
      ...
      <div style={{ background:"#0f1a24", border:"1px solid "+exec.accent+"44", borderRadius:"14px", padding:"24px", minWidth:"300px", maxWidth:"420px", width:"100%" }}>
        ...
        <div className="flex gap-2">
          <Button variant="secondary" className="flex-1" onClick={onClose}>Cancel</Button>
          <Button className="flex-1" style={{background:exec.accent}} onClick={()=>{ onSave([...selected]); onClose(); }}>Save</Button>
        </div>
      </div>
    </div>
  );
...
function LayerView({ layers }: { layers: any[] }) {
  return (
          ...
          <div key={layer.id} style={{ borderRadius:"12px", border:"1px solid "+layer.accent+"33", background:layer.color+"44", overflow:"hidden" }}>
            ...
            <div style={{ display:"flex", flexWrap:"wrap" }}>
              <div style={{ flex:"1 1 340px", padding:"16px 20px", borderRight:"1px solid "+layer.accent+"15", borderBottom:"1px solid "+layer.accent+"15" }}>
                ...
                {desc.activities.map((a: any,i: number)=>(
              ...
              <div style={{ flex:"0 1 260px", display:"flex", flexDirection:"column" }}>
                <div style={{ padding:"16px 20px", borderBottom:"1px solid "+layer.accent+"15" }}>
                  ...
                  <div style={{ display:"flex", flexDirection:"column", gap:"5px" }}>
                    {desc.outputs.map((o: any,i: number)=>(
    ...
    <div style={{ display:"flex", flexDirection:"column", gap:"32px" }}>
      ...
      <div style={{ background:"#0a1a14", border:"1px solid #3ecf8e33", borderRadius:"14px", overflow:"hidden" }}>
        ...
        <div style={{ padding:"16px 20px", display:"flex", flexWrap:"wrap", gap:"10px" }}>
          {signal.items.map((item: any,i: number)=>(
      ...
      <div style={{ background:"#0b1320", border:"1px solid #1e293b", borderRadius:"14px", padding:"22px 28px" }}>
        ...
        <div style={{ display:"flex", gap:"12px", flexWrap:"wrap" }}>
          {["PSE goes on-site → observes real friction","Configures platform for that church","Agents run → data returns to Gloo Brain","Brain gets smarter for next church","Next PSE deploys faster with better model","Repeat"].map((step: any,i: number,arr: any[])=>(
...
export default function MatrixOrg() {
  ...
  const [jdOpen,     setJdOpen]     = useState<any>(null); // { layerId, layer }
  ...
  const [assignOpen, setAssignOpen] = useState<any>(null); // exec object

  const updateLayerLead    = (id: string, v: string) => setOrg(o=>({...o,layers:o.layers.map((l: any)=>l.id===id?{...l,lead:{...l.lead,name:v}}:l)}));
  const addLayer           = (layer: any) => { const id=uid(); setOrg(o=>({...o,layers:[...o.layers,{...layer,id}]})); setShowAddRow(false); };
  const removeLayer        = (id: string) => setOrg(o=>({...o,layers:o.layers.filter((l: any)=>l.id!==id)}));
  const moveLayer          = (id: string, dir: number) => setOrg(o=>{ const arr=[...o.layers]; const i=arr.findIndex((l: any)=>l.id===id); const j=i+dir; if(j<0||j>=arr.length)return o; [arr[i],arr[j]]=[arr[j],arr[i]]; return{...o,layers:arr}; });

  const updateProductName  = (id: string, v: string) => setOrg(o=>({...o,products:o.products.map((p: any)=>p.id===id?{...p,name:v}:p)}));
  const updateProductLead  = (id: string, v: string) => setOrg(o=>({...o,products:o.products.map((p: any)=>p.id===id?{...p,productLead:v}:p)}));
  const addProduct         = ({name,type,productLead}: {name: string; type: string; productLead: string}) => { const id=uid(); const cells: Record<string, string[]>={}; org.layers.forEach((l: any)=>{cells[l.id]=[];}); setOrg(o=>({...o,products:[...o.products,{id,name,type,productLead,cells}]})); setShowAddCol(false); };
  const removeProduct      = (id: string) => setOrg(o=>({...o,products:o.products.filter((p: any)=>p.id!==id)}));
  const moveProduct        = (id: string, dir: number) => setOrg(o=>{ const arr=[...o.products]; const i=arr.findIndex((p: any)=>p.id===id); const j=i+dir; if(j<0||j>=arr.length)return o; [arr[i],arr[j]]=[arr[j],arr[i]]; return{...o,products:arr}; });

  const updateCellName     = (pId: string, lId: string, idx: number, v: string) => setOrg(o=>({...o,products:o.products.map((p: any)=>{if(p.id!==pId)return p;const n=[...(p.cells[lId]||[])];n[idx]=v;return{...p,cells:{...p.cells,[lId]:n}};})}));
  ...
  // Executive ops
  const addExecutive = () => {
    const newExec = { id:uid(), name:"New Leader", role:"Executive Role", color:"#1a2030", accent:"#60a5fa", layers:[] as string[] };
    setOrg(o=>({...o, executives:[...(o.executives||[]), newExec]}));
    setAssignOpen(newExec);
  };
  const removeExecutive = (id: string) => setOrg(o=>({...o, executives:(o.executives||[]).filter((e: any)=>e.id!==id)}));
  const updateExecName = (id: string, v: string) => setOrg(o=>({...o, executives:(o.executives||[]).map((e: any)=>e.id===id?{...e,name:v}:e)}));
  const updateExecRole = (id: string, v: string) => setOrg(o=>({...o, executives:(o.executives||[]).map((e: any)=>e.id===id?{...e,role:v}:e)}));
  const updateExecLayers = (id: string, layers: string[]) => setOrg(o=>({...o, executives:(o.executives||[]).map((e: any)=>e.id===id?{...e,layers}:e)}));
  ...
  return (
    <div style={{ fontFamily:"Georgia, Times New Roman, serif", background:"#070d14", minHeight:"100vh", color:"#e2e8f0", padding:"28px 20px" }}>
      ...
      <div className="flex justify-center mb-5">
        <TabGroup value={view} onValueChange={(v: string) => setView(v)} size="sm">
          <TabGroupItem value="people">👥  People</TabGroupItem>
          <TabGroupItem value="layers">⚙️  How Layers Work</TabGroupItem>
          <TabGroupItem value="agenticOS">🧠  Agentic OS</TabGroupItem>
        </TabGroup>
      </div>
          ...
          <thead>
            <tr>
              <th style={{ width:"140px", padding:"10px 12px", textAlign:"left", fontSize:"9px", letterSpacing:"3px", color:"#334155", fontFamily:"Courier New, monospace", fontWeight:"400", borderBottom:"1px solid #1e293b", borderRight:"1px solid #1e293b", verticalAlign:"middle" }}>
                ...
                <Button variant="ghost" size="sm" onClick={addExecutive}
                  className="font-mono tracking-wider text-[9px] border border-dashed border-slate-700 text-slate-500 hover:border-slate-200 hover:text-slate-200">+ ADD</Button>
              </th>
              ...
              <th style={{ padding:"14px 12px", borderBottom:"1px solid #1e293b", background:"transparent", verticalAlign:"middle" }}>
                <Button variant="ghost" size="sm" onClick={()=>setShowAddCol(true)}
                  className="font-mono tracking-wider text-[11px] border border-dashed border-blue-400/35 text-blue-400/60 hover:border-blue-400 hover:text-blue-400">+ Col</Button>
              </th>
            </tr>
          </thead>
          <tbody>
            {(() => {
              ...
              const segments: ({ exec: any; rowSpan: number } | null)[] = [];
              let i = 0;
            ...
            <tr>
              <td colSpan={org.products.length + 3} style={{ padding:"12px 16px", background:"transparent" }}>
                <Button variant="ghost" size="sm" onClick={()=>setShowAddRow(true)}
                  className="font-mono tracking-wider text-[11px] border border-dashed border-emerald-400/35 text-emerald-400/60 hover:border-emerald-400 hover:text-emerald-400">+ Add Row</Button>
              </td>
            </tr>
          </tbody>
        </table>
